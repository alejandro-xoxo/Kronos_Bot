...
| `v2.5` (en progreso, sin tag todavía) | Umbral del circuit breaker configurable desde el panel de control (`settings.circuit_breaker_pct`), en vez de fijo en 6% dentro del workflow. Implementado en `feature/circuit-breaker-editable`, **sin mergear a `develop` ni probado contra el stack dev/real todavía** — ver estado más abajo. | 2026-09-01 |
...
- [ ] Ver la interpretación por Gemini disparada por una instrucción de seguimiento real del grupo (`GEMINI_API_KEY` cargada y workflow activo desde el 31/08/2026, todavía no se probó contra una instrucción real)
- [ ] Umbral del circuit breaker editable desde el panel (`v2.5`, rama `feature/circuit-breaker-editable`): implementado, sin mergear a `develop` y sin probar contra el stack dev o real — no se pudo levantar el stack dev porque producción está corriendo, esperando mensajes del grupo, y ambos compiten por el mismo callback de Telegram (ver nota de dev/prod no simultáneos)
