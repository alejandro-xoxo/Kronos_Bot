...
# El tamaño/posición de Dashboard/Telegram/MT4 se calculan en píxeles
# reales contra el monitor efectivo de cada ventana, en vez de con
# `windowrule = size %/move %` de kronos-layout.conf: cuando el
# monitor secundario (HDMI-A-1) no está conectado, Hyprland calcula
# esos porcentajes contra el monitor "asignado" al workspace en vez de
# contra donde el workspace realmente termina cayendo, y las ventanas
# aparecen con coordenadas negativas, fuera de pantalla — bug real,
# reproducido 2026-08-24. Ver scripts/kronos-posicionar-ventanas.sh
# para el detalle de por qué esa lógica vive en un archivo aparte,
# invocado por RUTA (no embebido como string vía `declare -f` + `bash
# -c` dentro de systemd-run: systemd interpreta cualquier `$variable`
# del string como referencia a env del *unit*, no del bash interno, y
# los sustituye por vacío antes de ejecutar nada — bug real,
# reproducido 2026-08-24, el reposicionamiento corría con exit 0 sin
# mover ni redimensionar ninguna ventana).
if command -v hyprctl >/dev/null 2>&1; then
    ...
    systemd-run --user --unit="kronos-layout-$$" -- \
        bash "${REPO_DIR}/scripts/kronos-posicionar-ventanas.sh" >/dev/null 2>&1 || true
fi
