...
- ~~**Ciclo de cierre y registro en Google Sheets** (Fase 7)~~ —
  **descartado 2026-08-29, decisión explícita del usuario.** Ya no
  hace falta: `PVG_kronos` (Calendario/Crecimiento, con datos reales
  vía `daily_pnl`/`GET /api/registros`) cubre el mismo rol de
  "registro legible para humanos" que iba a cumplir Sheets. No se
  implementa Sheets nunca — ver Fase 7 más abajo, redefinida.
...
3. ~~Registro en Google Sheets de los cierres (Fase 7)~~ — **descartado
   2026-08-29**, reemplazado por `PVG_kronos`. Nada que hacer acá.
...
- ✅ Fase 7 (redefinida 2026-08-29) — cierre y registro legible. El
  consumidor de `orders/closed/*.json` (detección TP/SL) está
  mergeado, compilado y activo de punta a punta en producción real
  (verificado 2026-08-21, ver nota más abajo). El registro legible ya
  no es Google Sheets (descartado, decisión explícita del usuario) —
  lo cubre `PVG_kronos` (Calendario/Crecimiento, dev por ahora, ver
  sección de integración de PVG_kronos más arriba).
